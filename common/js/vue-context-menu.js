...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    ...
    clickDocumentHandler: function clickDocumentHandler(e) {
      this.$emit('update:show', {show:false,dataset:undefined});
    },
    ...
    contextMenuHandler: function contextMenuHandler(e) {
      ...
      // 15为此时最顶层到该层元素的距离。节省一点时间
      for (let i = 0; i < paths.length - 15; i++) {
          if (paths[i].dataset && paths[i].dataset['iIndex']!==undefined){
            dataset = paths[i].dataset
            break
          }
      }
      ...
      this.y = e.clientY;
      this.layout();
      this.$emit('update:show', {show:true,dataset});
      e.preventDefault();
    },
