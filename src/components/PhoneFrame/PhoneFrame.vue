  ...
  <view class="phone" :style="phoneSize" ref="contextMenuTarget">
    <view v-show="showComponentPreview" class="tip-content-show">
      <view class="tip-content-show-content">
        ...
        <template v-if="componentPreviewName === 'myComponent'">
          <my-components-entity
            :list="myComponentPreviewData"
          ></my-components-entity>
        </template>
      </view>
    </view>
    <view class="phone-top">
      <view style="flex: 1;text-align: left;"><el-icon><CloseBold /></el-icon>&nbsp;无服务</view>
      <view style="flex: 1; text-align: center">{{ time }}</view>
      <view style="flex: 1;text-align: right;margin-right: 20px;"
        ><el-icon color="#aeb1b7">
    <Share />
    
  </el-icon>&nbsp;&nbsp;<el-icon><ChatLineRound /></el-icon>&nbsp;
  <el-icon><Connection /></el-icon></view>
    </view>
    <view class="phone-top-blok"> </view>

    <!-- :style="[computedClassToStyle(phoneClass),phoneStyle]" 关键！！！-->
...
$windowsWidth: calc($phoneWidth / 0.2);

$onePx: calc(calc($windowsWidth / 750) * 0.2);
